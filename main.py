...
#
# Initialize button and text using class
class Button():
...
class StartButton(Button):
    def __init__(self, screen, text, x, y):
        super().__init__(screen, text, x, y, [(26, 173, 25), (158, 217, 157)], False)


    def click(self, game, level):
       self.level = level
        if self.enable:
            game.start(self,level)
            game.winner = None
            self.msg_image = self.font.render(self.text, True, self.text_color, self.button_color[1])
            self.enable = False
            return True
...
class EasyButton(Button):
    def __init__(self, screen, text, x, y):
        super().__init__(screen, text, x, y, [(135, 206, 250), (176, 196, 222)], True)
    ...
    def click(self, game):
        if self.enable:
            ...
            self.msg_image = self.font.render(self.text, True, self.text_color, self.button_color[1])
            self.enable = False
            MediumButton.enable = False
            DifficultyButton.enable = False
            return True
        return False
...
class Game():
    def __init__(self, caption):
        ...
        self.buttons.append(GiveupButton(self.screen, 'Giveup', MAP_WIDTH + 30, BUTTON_HEIGHT + 275))
        #
        self.buttons.append(EasyButton(self.screen, 'Easy', MAP_WIDTH + 30, BUTTON_HEIGHT + 45))
        self.buttons.append(MediumButton(self.screen, 'Normal', MAP_WIDTH + 30, BUTTON_HEIGHT + 110))
        self.buttons.append(DifficultyButton(self.screen, 'Hard', MAP_WIDTH + 30, BUTTON_HEIGHT + 175))
        #
        self.is_play = False
    ...
    def play(self):
        ...
        if self.is_play and not self.isOver():
            if self.useAI:
                x, y = self.AI.findBestChess(self.map.map, self.player, self.level)
    ...
    def showWinner(self):
        def showFont(screen, text, location_x, locaiton_y, height):
            ...
            font_image = font.render(text, True, (205, 85, 85), (255, 255, 255))
...
game = Game("Gobang CHESS " + GAME_VERSION)
