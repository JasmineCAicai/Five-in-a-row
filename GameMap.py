...
REC_SIZE = 50
CHESS_RADIUS = REC_SIZE // 2 - 2  # 23
CHESS_LEN = 15  # 15 rows, center(7, 7)
MAP_WIDTH = CHESS_LEN * REC_SIZE  # 750
MAP_HEIGHT = CHESS_LEN * REC_SIZE  # 750

INFO_WIDTH = 200  # Information area
BUTTON_WIDTH = 140  # All buttons' width
BUTTON_HEIGHT = 50  # All buttons' height

SCREEN_WIDTH = MAP_WIDTH + INFO_WIDTH  # 950
SCREEN_HEIGHT = MAP_HEIGHT  # 750


# In our game, player_one is user, player_two is AI
...
class Map():
    ...
    def drawSelection(self, screen):
        def showOption(screen, text, location_x, locaiton_y, height):
            Option = pygame.font.SysFont(None, height)
            Option_image = Option.render(text, True, (47, 79, 79), (255, 255, 255))
            Option_image_rect = Option_image.get_rect()
            Option_image_rect.x = location_x
            Option_image_rect.y = locaiton_y
            screen.blit(Option_image, Option_image_rect)

        str = 'select level'
        showOption(screen, str, MAP_WIDTH + 30, 40, 35)